...
export namespace Roles {
	...
	function satisfiesFinancialController(permissions: Permissions, organizationId?: string): boolean {
		return (
			...
			(!!permissions[organizationId ?? ""]?.organization?.write || !!permissions["*"]?.organization?.read) &&
			(!!permissions[organizationId ?? ""]?.organization?.read || !!permissions["*"]?.organization?.write) &&
			...
			(!!permissions[organizationId ?? ""]?.delegation?.read || !!permissions["*"]?.delegation?.write) &&
			(!!permissions[organizationId ?? ""]?.delegation?.write || !!permissions["*"]?.delegation?.read)
		)
	}
	function satisfiesUser(permissions: Permissions, organizationId?: string): boolean {
		return !!organizationId && !!permissions[organizationId]?.user?.write
	}
}
